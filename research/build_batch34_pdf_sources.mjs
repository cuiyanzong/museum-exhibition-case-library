...
const ledger = JSON.parse(fs.readFileSync(path.join(root, 'research/expansion_1000_batch34_candidates.json'), 'utf8'));
const contents = JSON.parse(fs.readFileSync(path.join(root, 'research/batch34_case_content.json'), 'utf8'));
const pdfRoot = path.join(root, 'research/expansion_1000_batch34_pdfs');
const assetFsRoot = '/home/ubuntu/webdev-static-assets/expansion-1000-batch34';
const assetRoot = '/webdev-static-assets/expansion-1000-batch34';
...
function makeMain(entry, content) {
  ...
  return `// 第34批田野档案室PDF。仅使用人工审核的已建成展厅摄影与可追溯责任链。
...
for (const content of contents) {
  ...
  const manifest = {
    ...
    hard_constraints: ['two verified interior photographs', 'Chinese CJK text'],
