...
const pdfRoot = path.join(root, 'research/expansion_1000_batch33_pdfs');
const assetFsRoot = '/home/ubuntu/webdev-static-assets/expansion-1000-batch33';
const assetRoot = '/webdev-static-assets/expansion-1000-batch33';
...
function makeSourceRows(content) { return content.sources.map((url, index) => `[来源 ${index + 1}], [${link(url, url)}]`).join(',\n  '); }
function makeMain(entry, content) {
  ...
  return `// 第33批田野档案室PDF。仅使用人工审核的已建成展厅摄影与可追溯责任链。
