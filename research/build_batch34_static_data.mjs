...
const pdfs = JSON.parse(fs.readFileSync(path.join(root, 'research/batch34_uploaded_pdf_manifest.json'), 'utf8')).assets;
const output = path.join(root, 'client/src/data/caseExpansion1000Batch34.ts');
...
const cases = contents.map((content) => {
  ...
  if (!content.imageUrls.every((url) => url.startsWith('/manus-storage/'))) {
    throw new Error(`Non-persistent image URL for ${content.id}`);
  }
  return {
    id: content.id,
    region: entry.region,
...
const source = `import type { CaseItem } from "@/data/cases";\n\nexport const expansion1000Batch34Cases: CaseItem[] = ${JSON.stringify(cases, null, 2)};\n`;
