...
const pdfRoot = path.join(root, 'research/expansion_1000_batch30_pdfs');
const assetFsRoot = '/home/ubuntu/webdev-static-assets/expansion-1000-batch30';
const assetRoot = '/webdev-static-assets/expansion-1000-batch30';
...
function makeSourceRows(content) {
  return content.sources.map((url, index) => `[来源 ${index + 1}], [${link(url, url)}]`).join(',\n  ');
}
function makeMain(entry, content) {
  const design = link(entry.design_source, '设计／展陈责任来源');
  const museum = link(entry.museum_source, '馆方／当前运营来源');
  const analysis = content.analysis.map(([label, text], index) => `== 0${index + 1}｜${label}\n\n${typstText(text)}\n`).join('\n');
  return `// 第30批田野档案室PDF。仅使用经人工审核的已建成展厅摄影与可追溯责任链。
...
${typstText(content.brief)} 本报告将其作为${typstText(content.type)}研究；展陈责任、当前运营与叙事范围以设计／展陈责任方和馆方公开来源交叉核验。${design} ${museum}
...
#table(columns: (1.25fr, 2.75fr), stroke: 0.4pt + luma(175), inset: 7pt,
  [责任／事实], [可追溯来源],
  [展陈设计／制作], [${typstText(entry.studio)}。${design}],
  [馆方叙事与运营], [馆方页面提供项目叙事、访问或当前运营线索。${museum}],
...
for (const content of contents) {
  ...
  const manifest = { task_kind: 'new-document', output: 'pdf', signals: [], hard_constraints: ['two verified interior photographs', 'Chinese CJK text'], content_features: { code_blocks: 0, defined_terms: 0, portrait_images: 0 } };
  ...
  if (!fs.existsSync(path.join(dir, 'report-theme.typ'))) execFileSync('python3', ['/home/ubuntu/skills/typst-pdf-maker/scripts/prepare_document.py', 'report', dir, '--title', content.title, '--subtitle', content.subtitle, '--author', 'Manus AI'], { cwd: root, stdio: 'pipe' });
