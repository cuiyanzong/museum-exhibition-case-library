...
const statusPath = path.join(root, 'research/batch28_pdf_compile_status.json');
const results = [];
...
for (const id of fs.readdirSync(pdfRoot).sort()) {
  ...
  const run = spawnSync('typst', ['compile', '--root', '/home/ubuntu', '--diagnostic-format', 'short', input, output], {
    encoding: 'utf8',
  });
