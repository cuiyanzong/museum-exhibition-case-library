...
import { expansion1000Batch32Cases } from "./caseExpansion1000Batch32";
import { caseLibrary } from "./caseLibrary";

describe("第三十二批案例数据", () => {
  it("提供5个可唯一识别、具备两张持久化现场图、PDF与五层分析的案例", () => {
    expect(expansion1000Batch32Cases).toHaveLength(5);
    expect(new Set(expansion1000Batch32Cases.map((item) => item.id)).size).toBe(5);

    for (const item of expansion1000Batch32Cases) {
  ...
  it("被追加至全局案例库尾部，不改变此前批次的相邻导航顺序", () => {
    ...
    expect(caseLibrary.slice(-15, -10).map((item) => item.id)).toEqual(expansion1000Batch32Cases.map((item) => item.id));
