...
import { expansion1000Batch30Cases } from "./caseExpansion1000Batch30";
import { caseLibrary } from "./caseLibrary";

describe("第三十批案例数据", () => {
  it("提供20个可唯一识别、具备完整资源和五层分析的案例", () => {
    expect(expansion1000Batch30Cases).toHaveLength(20);
    expect(new Set(expansion1000Batch30Cases.map((item) => item.id)).size).toBe(20);

    for (const item of expansion1000Batch30Cases) {
  ...
  it("被追加至全局案例库尾部，不改变此前批次的相邻导航顺序", () => {
    ...
    expect(caseLibrary.slice(-40, -20).map((item) => item.id)).toEqual(expansion1000Batch30Cases.map((item) => item.id));
